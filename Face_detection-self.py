...
smile_cascade = cv2.CascadeClassifier('/home/shaxpy/Desktop/opencv-master/data/haarcascades/haarcascade_smile.xml') 
def detect(gray,frame):# get coordinates of rectangles for faces and detect eyes
    ...
    for(x,y,w,h) in faces:
        ...
        roi_gray=gray[y:y+h,x:w] #region of interest-roi
        roi_color=frame[y:y+h,x:w]
        eyes=eye_cascade.detectMultiScale(roi_gray,1.1,3)
        for(ex,ey,ew,eh) in eyes:
            cv2.rectangle(roi_color, (ex,ey),(ex+ew,ey+eh),(0,255,0),2)
        smiles = smile_cascade.detectMultiScale(roi_gray, 1.7, 26) 
        for(sx,sy,sw,sh) in smiles:            cv2.rectangle(roi_color, (sx,sy),(sx+sw,sy+sh),(0,0,255),2)
    return frame
