...
canny=canny2(lane_image)
crop=region(canny)
lines=cv2.HoughLinesP(crop,2,np.pi/180,100,np.array([]),minLineLength=40,maxLineGap=5)
average=averageim(lane_image,lines)

line_image=dlines(lane_image,average)
combine=cv2.addWeighted(lane_image,0.8,line_image,1,1)# gamma values- to add to the sum
